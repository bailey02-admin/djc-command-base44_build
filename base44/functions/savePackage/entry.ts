...
Deno.serve(async (req) => {
  try {
    ...
    if (!data.name || data.base_price == null) return Response.json({ error: "name and base_price required" }, { status: 400 });

    const svc = base44.asServiceRole;
    let result;
    if (action === "create") {
      result = await svc.entities.Package.create(data);
    } else if (action === "update") {
      if (!id) return Response.json({ error: "id required for update" }, { status: 400 });
      result = await svc.entities.Package.update(id, data);
    } else {

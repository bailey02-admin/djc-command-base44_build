...
Deno.serve(async (req) => {
  try {
    ...
    const tokens = await base44.asServiceRole.entities.AuthToken.filter({ token_hash: tokenHash, type: 'password_reset' });
    const authToken = tokens[0];

    if (!authToken) return Response.json({ error: 'Invalid or expired reset link' }, { status: 400 });
    if (authToken.used_at) return Response.json({ error: 'This reset link has already been used' }, { status: 400 });
    if (new Date(authToken.expires_at) < new Date()) return Response.json({ error: 'This reset link has expired. Please request a new one.' }, { status: 400 });
